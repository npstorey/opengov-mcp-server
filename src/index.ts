...
async function createServer() {
  ...
  if (typeof (mcpServer as any).onError === 'function') {
...
app.get(ssePath, async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.log(`[MCP Server] SSE transport connected with session ${transport.sessionId}`);
  } catch (err) {
    console.error('[MCP Server] Error connecting MCP server to SSE transport:', err);
...
app.post(messagesPath, async (req: Request, res: Response) => {
  ...
  if (!sessionId || typeof sessionId !== 'string') {
    console.error('[MCP Server] POST /mcp/messages missing or invalid sessionId');
...
app.get('/', (_req: Request, res: Response) => {
  res.status(200).send('OpenGov MCP Server is running. MCP endpoint at /mcp/sse and /mcp/messages.');
});

app.listen(port, '0.0.0.0', () => {
  console.log(`🚀 MCP server listening on port ${port}. SSE at ${ssePath}, Messages at ${messagesPath}`);
});
